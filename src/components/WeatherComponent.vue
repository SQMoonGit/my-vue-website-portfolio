      ...
      <v-col lg="2" dense>
        <v-card height="250">
          <v-card-text class="pt-3"
            >{{ forecast["name"] }}, {{ state }}</v-card-text
          >
          <v-card-text v-if="forecast.sys.country">{{
          ...
          <v-card-text
            >Current Temp: {{ forecast["main"].temp }}°{{
              tempUnit
            }}</v-card-text
          >
          <v-card-text
            >High: {{ forecast["main"].temp_max }}°{{ tempUnit }} / Low:
            {{ forecast["main"].temp_min }}°{{ tempUnit }}</v-card-text
          >
      ...
      <v-col lg="2" dense>
        <v-card height="250">
          ...
          <v-card-text v-for="(day, index) in fiveDayForecast" :key="index">
            ...
            {{ day["main"].temp_max }}°{{ tempUnit }} /
            {{ day["main"].temp_min }}°{{ tempUnit }}
          </v-card-text>
...
export default class WeatherComponent extends Vue {
  ...
  private weatherIconArray: object = {
    Clear: "fa-sun",
    Clouds: "fa-cloud",
    Rain: "fa-cloud-rain"
  };
  ...
  public getWeather() {
    this.$http
      .get(
        `${this.apiURL}/weather?q=${this.city},${this.state}&units=imperial&appid=${this.weatherKey}`
      )
      .then(
        result => {
        ...
        error => {
          ...
          console.log(
            "[ERROR] getWeather - WeatherComponent - GET/" +
              this.city +
              this.state
          );
        }
    ...
    this.$http
      .get(
        `${this.apiURL}/forecast?q=${this.city},${this.state}&units=imperial&appid=${this.weatherKey}`
      )
      .then(
        result => {
          if (result.status === 200 && result.data) {
